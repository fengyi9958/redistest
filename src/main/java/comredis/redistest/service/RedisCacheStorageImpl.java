...
import comredis.redistest.util.RedisClient;
...
public class RedisCacheStorageImpl<V> implements RedisCacheStorage<String, V> {
    ...
    public V hget(String cacheKey, String key) {  
        ...
        return (V) JSONObject.toJavaObject((JSON) jValue, UserEntity.class);
    }  
    /** 
     * 获取哈希类型的数据 
     * @param cacheKey 
        ...
        try {  
            ...
            if(null !=map){  
                for(Map.Entry<String, String> entry : map.entrySet()){  
                    ...
                    result.put(JSONObject.toJSONString(entry.getKey()), (V)JSONObject.toJavaObject((JSON) JSON.parseObject(entry.getValue()),UserEntity.class));
                }
            }  
        } catch (JedisException e) {  
            LOG.info("client can't connect server");  
